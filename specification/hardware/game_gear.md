...
二次確認では、SMS Power の `VDP Registers` に `SMS2 / GG` として記載されている `$05`, `$06`, `$07` を再確認し、少なくとも sprite attribute table base、sprite pattern generator base、backdrop color の系統は Game Gear でも同種の考え方でよいことを確認した。
...
このうち二次確認で特に裏が取れた差分は次。

- 可視解像度は `160x144`
- パレットは `4096 colors`
- 同時表示は `32 colors`
- VDP register 文書上、`SMS2 / GG` として sprite table / sprite pattern / backdrop color の項目が分岐記載されている
...
- これは「Game Gear に palette line が無い」という意味ではない
- 二次確認時点では、GGを SMS Mode 4 の完全な palette-line 運用まで厳密化する資料よりも、`SMS2 / GG` のレジスタ互換性を示す資料のほうが明瞭だった
- そのため、初版仕様では `まず破綻しない安全側の固定` として BG/Sprite を16色ずつに分けている
...
- `SMS2 / GG` は `$05` の mask bit 挙動が SMS1 と異なり、unused 扱いになっている
- よって、Game Gear は SMS1 特有の mask bit 癖を前提にしないほうが安全
...
- base address 切替そのものは Game Gear 系でも残っている
- ただし SMS1 にある mask bits の癖は `SMS2 / GG` では unused 側として扱われる
